...
export default function AssignedOrderForm({
  open,
  onClose,
  edit,
  openRow,
  ...
  const RecyclerFormSchema = yup.object().shape({
    ...
    expectedPickupDate: yup
      .string()
      .required("Expected Pickup Date is required"),
  ...
  useEffect(() => {
    if (edit && openRow) {
      ...
      setValue("expectedPickupDate", openRow?.expectedPickupDate);
      setValue("finalPrice", openRow?.finalPrice);
    } else {
      reset();
    }
          ...
          <form onSubmit={handleSubmit(handleClick)}>
            <Stack spacing={3}>
              ...
              <TextField
                ...
                type="date"
                {...register("expectedPickupDate")}
                error={Boolean(errors.expectedPickupDate)}
                helperText={errors.expectedPickupDate?.message}
              />
